...
module.exports = {
  ...
  getRootUrlFromFunctionName: function(func){
    return '/' + this.toSnakeCase(this.getFunctionNameFromFunction(func));
  },
  containsModelArg: function(args){
    return args.filter(Filters.model).length > 0 ? true : false;
  },
  containsModelsArg: function(args){
    return args.filter(Filters.models).length > 0 ? true : false;
  },
  ...
  injectController: function(controller){
    var args = this.getArgStringsFromFunction(controller)
      , controllerArgs = []
      , routes
      , root = this.getRootUrlFromFunctionName(controller);
    ...
    if (this.containsModelsArg(args)){
      controllerArgs.push(this.models);
    }
  ...
  injectAllControllers: function(){
    ...
    fs.readdir(this.controllersDir, function(err, files){
      files.filter(function(file){
        ...
        self.injectController(controller);
      });
    });
  }
};